...
export function addRequestInterceptor(service: AxiosInstance, successCode: number) {
  // request拦截器
  ...
  service.interceptors.response.use(
    async (res) => {
      ...
      if (code === 401) {
        ...
        if ((!isRelogin.show || isRelogin.message !== message) && location.pathname !== LoginUrl) {
          isRelogin.show = true
        ...
        return [{ message: '无效的会话，或者会话已过期，请重新登录。' }, axiosData.data, axiosData]
      }
